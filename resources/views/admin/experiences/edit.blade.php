...
@section('pageTitle', 'Edit Work Experience')
...
<div class="form-card">
  <div class="form-header">
    ...
    <div>
      ...
      <p>Ubah data pengalaman kerja yang sudah ada.</p>
    </div>
  </div>
    ...
    <form method="POST" action="{{ route('admin.experiences.update', $experience->id) }}">
      ...
      <div class="form-grid">
        <div class="field">
          ...
          <input type="text" name="company" value="{{ old('company', $experience->company) }}" required placeholder="e.g. Google, Tokopedia">
        </div>
        <div class="field">
          <label>Lokasi</label>
        <!-- Di edit.blade.php -->
          <input type="text" name="location" value="{{ old('location', $experience->location ?? 'Remote') }}" placeholder="Kota, Negara">
        </div>
        <div class="field">
          <label>Posisi / Role <span style="color:#ef4444">*</span></label>
          <input type="text" name="role" value="{{ old('role', $experience->role) }}" required placeholder="e.g. Frontend Developer">
        </div>
        <div class="field">
          <label>Periode <span style="color:#ef4444">*</span></label>
          <input type="text" name="period" value="{{ old('period', $experience->period) }}" required placeholder="e.g. Jan 2023 - Sekarang">
        </div>
        <div class="field full">
          <label>Deskripsi Pekerjaan</label>
          <textarea name="details" placeholder="Jelaskan tanggung jawab dan pencapaian Anda...">{{ old('details', $experience->details) }}</textarea>
        </div>
      </div>
